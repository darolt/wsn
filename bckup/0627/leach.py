...
def run_mte(nodes):
  ...
  for round in range(0, MAX_ROUNDS):
    print("round %d, total remaining energy: %f" % (round, nodes.get_remaining_energy()))
...
def run_direct_communication(nodes):
  ...
  nb_alive_nodes = []
  for round in range(0, MAX_ROUNDS):
    print("Total remaining energy: %f" % (nodes.get_remaining_energy()))
    if not nodes.someone_alive():
      break
...
def run_fcm(nodes):
  ...
  for round in range(0, MAX_ROUNDS):
    ...
    if not 'clusters' in locals(): # clusters do not change in FCM
      clusters = nodes.split_in_clusters()
    for cluster in clusters:
      sleep_scheduling_pso(cluster, base_station)
    nodes.broadcast_next_hop()
    nodes.run_round()
...
if __name__ == '__main__':
  ...
  curves = []
  if RUN_DC:
    curves.append(run_direct_communication(nodes))
    reset_nodes(nodes)
  if RUN_MTE:
    ...
    curves.append(run_mte(nodes))
    reset_nodes(nodes)
  if RUN_LEACH:
    nodes.set_zero_cost_aggregation(1)
    curves.append(run_leach(nodes))
    reset_nodes(nodes)
  if RUN_FCM:
    nodes.set_zero_cost_aggregation(1)
    curves.append(run_fcm(nodes))
    reset_nodes(nodes)
